...
def _build_editor_tab(actions) -> None:
    ...
    with dpg.tab_bar(tag=TAGS["editor_tabs"]):
        ...
        with dpg.tab(label="Evolutions"):
                with dpg.group(horizontal=True):
                    dpg.add_text("Method")
                    dpg.add_combo(["EVO_LEVEL", "EVO_ITEM", "EVO_TRADE", "EVO_FRIENDSHIP"], tag="evo_method", width=170, callback=actions.on_evolution_method_change)
                    dpg.add_text("Level", tag="evo_level_label")
                    dpg.add_input_int(tag="evo_level_param", width=100, min_value=0, max_value=100, min_clamped=True, max_clamped=True, default_value=16, show=True, callback=actions.mark_dirty)
                    dpg.add_text("Friendship", tag="evo_friendship_label")
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_combo(["MOVE_TACKLE"], tag="evo_condition_value", width=260, callback=actions.mark_dirty, show=False)
                    dpg.add_input_int(tag="evo_condition_value_int", width=110, callback=actions.mark_dirty, show=False, min_value=0, min_clamped=True)
                    dpg.add_input_text(tag="evo_condition_param", width=260, callback=actions.mark_dirty, show=False)
                with dpg.group(horizontal=True):
                    dpg.add_button(label="Add condition", callback=actions.add_evolution_condition_row)
                    dpg.add_button(label="Update condition", callback=actions.update_evolution_condition_row)
                    dpg.add_button(label="Remove condition", callback=actions.remove_evolution_condition_row)
                dpg.add_listbox([], tag="evo_condition_rows", width=700, num_items=3, callback=actions.select_evolution_condition_row, show=False)
                with dpg.group(horizontal=True):
